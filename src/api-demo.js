// Uma API de mentira para ensaiar e testar o slide "api" sem nenhum serviço de verdade
// (`sagadeck ensaio-api` e a suíte de testes). Genérica de propósito: não imita nenhum serviço real.
//   POST /identity         client_id + client_secret -> JWT em { data: { token } }
//   POST /v1/upload        multipart -> { path_id }       POST /v1/ocr  { path_id } ou { content: base64 } -> { text }
//   POST /v1/embeddings    { input } -> vetor              POST /v1/index  (falha com mode "rows" + chunk_size > 0)
//   POST /v1/tts           { text } -> áudio WAV           POST /v1/stt  multipart -> { text }
...
export async function startMockApi({ clientId = "id-teste", clientSecret = "segredo-teste-123", statuses = ["STARTED", "RUNNING", "RUNNING", "FINISHED"], ttlSeconds = 1800, format = "json" } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // "Indexador": pdf em pedaços ou planilha linha a linha; a combinação errada falha (como na vida real)
    if (url.pathname === "/v1/index" && req.method === "POST") {
      const b = JSON.parse(raw || "{}");
      if (!b.path_id) return json(res, 400, { error: "path_id é obrigatório" });
      if (b.mode === "rows" && Number(b.chunk_size) > 0) return json(res, 422, { error: "indexação falhou: chunk_size não se aplica a mode=rows" });
      if (b.mode === "rows" && !(b.sheet && b.sheet.header_row)) return json(res, 422, { error: "indexação falhou: mode=rows exige sheet.header_row" });
      const content = state.files?.[b.path_id] || "";
      const n = b.mode === "rows" ? Math.max(1, content.split("\n").filter(Boolean).length - 1) : Math.max(1, Math.ceil(content.length / (Number(b.chunk_size) || 800)));
      return json(res, 200, { status: "indexed", mode: b.mode || "pdf", documents: n, index: b.index || "workshop" });
    }
