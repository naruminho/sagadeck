...
(function () {
  ...
  const LAYOUTS = {
    ...
    spotlight: [f.text("kicker", "Chapéu"), f.text("title", "Título"), f.text("image", "Imagem (arquivo ou link)", { hint: "As regiões usam porcentagens da imagem: 0 é o início, 100 é o fim." }),
      f.text("caption", "Legenda"), f.list("hotspots", "Detalhes em foco", obj([f.text("title", "Título do detalhe"), f.area("text", "Explicação"),
  ...
  const ELEMENT_KINDS = [
    ...
    { id: "image", label: "Imagem", is: (e) => e.image || e.image_prompt, tpl: () => ({ image: "", fit: "cover" }),
      fields: [f.text("image", "Arquivo ou link"), f.area("image_prompt", "Descrição para a IA gerar", { hint: "Escreva e clique em Gerar imagem agora." }),
        f.action("Gerar imagem agora", (e, btn) => CTX.generateImage?.(e, btn), { when: (e) => !e.image && "image_prompt" in e }),
